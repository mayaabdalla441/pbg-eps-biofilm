# eps-biofilm + vivarium-workbench — first-run agent guide

> **You are a coding agent (Claude Code / Cursor / …). A first-time user handed
> you this file to get them onto the `eps-biofilm` + vivarium-workbench
...
- **This workspace (`eps-biofilm`)** — a
  ...
  **typed, independently-wireable processes** you *compose* into whatever
  ...
  `pbg_eps_biofilm/` (`core.py` exposes `build_core()`); research state lives
...
```bash
git clone <this repo's URL> eps-biofilm
cd eps-biofilm
```

**Gate:** `workspace.yaml` and `pbg_eps_biofilm/core.py` exist.
...
**Gate:** `.venv/bin/python -c "import pbg_eps_biofilm; print('ok')"` prints
...
- **Registry** — the process and type definitions this workspace registers
  (from `pbg_eps_biofilm/core.py`'s `build_core()`).
...
- **`ImportError` for `pbg_eps_biofilm` in the dashboard** — the editable
  install didn't run or ran into the wrong venv. Re-run `uv pip install -e .`
  and confirm `.venv/bin/python -c "import pbg_eps_biofilm"`.
